<template>
  <div class="component-loadmore" ref="loadmore">
    ...
    <div class="loadmore__footer" v-show="showText">
      <span v-if="loading">
        <i class="we-icon we-icon--loading loadmore__loading"></i>
        <span>正在加载...</span>
      </span>
  ...
  export default {
    ...
    computed: {
      ...
      loadable() {
        ...
        return !this.started || this.hasNextPage
      },
    },
    watch: {
      options: {
    ...
    methods: {
       ...
       */
      scroll() {
        // global.console.log(this.hasNextPage)
      ...
      load() {
        ...
        const params = {
          page: this.page + this.times,
          pageSize: this.rows,
        }
        const options = Object.assign({}, this.options, {
          url: this.url,
          // 同一个请求，没有完成的请求将被取消
          // https://zhuanlan.zhihu.com/p/30302124
        ...
        axios.request(options).then((res) => {
          ...
          // } else if (data.pager) {
          //   this.count = data.pager.pageCount
          // }
          this.$emit('success', data.list, params.page)
